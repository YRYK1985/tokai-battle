...
export default function AboutPage() {
  return (
    <div style={{
      ...
      <Section title="サイト概要">
        「東海オンエア 動画バトル」は、日本を代表するYouTuberグループ・東海オンエアの全動画3,000本以上の中から、
        ファンの投票によってランキングを作る参加型サイトです。
        2人組の動画が表示され、「どっちの動画が好き？」を選ぶだけのシンプルな仕組み。
        あなたの一票一票が、リアルタイムでランキングに反映されます。
      </Section>
      ...
      <Section title="楽しみ方">
        ...
        ランキングは「全期間」と「年度別」で切り替えることができ、2013年の初期の動画から最新の動画まで、
        あらゆる時代の東海オンエアの名作を発見できます。
        「こんな動画あったな！」「これが1位なの！？」など、ファン同士で語り合うきっかけになれば嬉しいです。
        投票結果はXでシェアすることもできます。
      </Section>
      ...
      <Section title="対象動画について">
        ...
        グレーゾーン・エージェンシーの連載動画は投票対象から除外しています。
        動画データ（再生回数・いいね数）はYouTube Data APIから定期的に取得し、最新の状態に更新しています。
      </Section>

      <Section title="運営について">
        このサイトは、東海オンエア公認切り抜きチャンネル「東海ランキング」（チャンネル登録者数32万人）の
        運営者が個人で制作・運営しています。UUUM公認のもと、東海オンエアのファンコミュニティを盛り上げることを
        目的としています。
        <br /><br />
      ...
      <Section title="よくある質問">
        ...
        <br />
        はい、何回でも投票できます。投票するほどランキングの精度が上がります。
        <br /><br />
